...
- 自然な語尾の例: 「〜です。」「〜だ。」「〜である。」「〜します。」「〜信じる。」
- 各文は1つの語尾で完結し、語尾を二つ重ねない
- 一文で完結。複文は読点「、」で繋げて1文にまとめる
...
function normalizeCandidate(raw, locale = 'ja') {
  ...
  // ━━ 文末の「したい」強制連結を全パターン削除 ━━━━━━━━━━━━━━
  // どんな日本語語尾の直後に「したい」「したいです」「したい。」が連結してても、
  // 直前の語尾までを残し、余分な「したい」「したいです」を削ぎ落とす。
  //
  // 1. 句点・末尾記号を一旦剥がす
  trimmed = trimmed.replace(/[。.!！\s]+$/u, '');

  // 2. 末尾の「したい」「したいです」「したい思う」「したいと思う」「したいと思います」を全削除
  //    (語尾が「だ/である/ます/です/ある/いる/する/なる/できる/得る/歩む/積む/続ける/生きる/楽しむ/愛する/尊敬する/信じる/思う/感じる/学ぶ」など何であっても無条件で削る)
  for (let i = 0; i < 4; i++) {
    let prev = trimmed;
    trimmed = trimmed.replace(/したい(と思います|と思う|と思って(い|る)|です|でしょう)?$/u, '');
    trimmed = trimmed.replace(/[。.!！\s]+$/u, '');
    if (trimmed === prev) break;
  }

  // 3. 末尾が「し」「いし」「をし」のように残った場合、自然な「たい」連結を再構成 (例: 「学びをし」 → 「学びをしたい」)
  //    ただし「し」だけ取り残されてる場合は語尾削除して動詞を残す
  if (/(を|に|が|は|も|と)?[しじ]$/u.test(trimmed)) {
    trimmed = trimmed.replace(/[しじ]$/u, '');
  }

  // 4. 重複した「思います」「信じます」「できます」のループも掃除
  trimmed = trimmed.replace(/(思います|信じます|できます|頑張ります)+\1/gu, '$1');

  // 5. 末尾の助詞・接続助詞が残っているなら削除 (例: 「〜が」「〜を」「〜に」)
  trimmed = trimmed.replace(/[、,，]\s*$/u, '');

  // 6. 文末記号 (句点) を最終的に1つだけ付与
  trimmed = trimmed.replace(/[。.!！]+$/u, '').trim();

  // 7. 空になった (したい以外の本文がない) 場合は空文字を返す → padCandidates でドロップ
